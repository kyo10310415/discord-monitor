...
export async function monitorChannels(env: Env, options: MonitorOptions = {}): Promise<{
  ...
  try {
    ...
    // Log the check
    await env.DB.prepare(
      `INSERT INTO check_logs (channels_checked, alerts_sent, status, error_message, channel_details, inactive_count, inactive_channels_details)
       VALUES (?, ?, ?, ?, ?, ?, ?)`
    ).bind(
      channelsChecked,
      alertsSent,
      ...
      inactiveChannels.length > 0 ? JSON.stringify(inactiveChannels) : null
    ).run();
    ...
    await env.DB.prepare(
      `INSERT INTO check_logs (channels_checked, alerts_sent, status, error_message, channel_details, inactive_count, inactive_channels_details)
       VALUES (?, ?, 'error', ?, ?, ?, ?)`
    ).bind(channelsChecked, alertsSent, errors.join('; '), null, 0, null).run();
  }
